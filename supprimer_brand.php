...
if(!empty($_GET['id'])){
    ...
    $count="SELECT COUNT(id) exist FROM brand WHERE id_student=$id";
    $result_count = mysqli_query($bdd,$count);
    
    while ($result_column = mysqli_fetch_array($count));


    if($result_count['exist'] ==6){
    $supprimer="delete from brand where id=$id";
